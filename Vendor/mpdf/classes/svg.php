...
class SVG {
	...
	function svgImage($attribs) {
		...
		// mPDF 6 Added $this->mpdf_ref->interpolateImage
		if ( 'data:image' === substr($srcpath, 0, 10) ) { // && false !== strpos($srcpath, 'base64,')
			// $pos = strpos($srcpath, 'base64,');
			$dataEncoded = preg_replace('@data:image/(\w+);base64,@', '', $srcpath);
			// $source = imagecreatefromstring($imageData);
			$tempfile = '_tempBase64_' . md5($dataEncoded);
			if ( !file_exists(_MPDF_TEMP_PATH . $tempfile) ) {
				$fp = fopen(_MPDF_TEMP_PATH . $tempfile, "w+");
				fwrite($fp, base64_decode($dataEncoded));
				fclose($fp);
			}
			$srcpath = _MPDF_TEMP_PATH . $tempfile;
		}
